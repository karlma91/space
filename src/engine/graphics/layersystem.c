...
drawbl_dualspr *state_add_dualsprite(STATE_ID state_id, int layer, SPRITE_ID spr_id, cpVect pos, cpVect size, Color col1, Color col2)
{
	...
	return (drawbl_dualspr *)state_add_drawable(state_id, layer, (draw_callback)draw_dualsprite, &dualspr, sizeof dualspr);
}

sprite_ext *state_add_sprite(STATE_ID state_id, int layer, SPRITE_ID id, float w, float h, cpVect p, float a, Color col)
{
	sprite_ext sprx;
	sprx.angle = a;
	sprx.pos = p;
	...
	return (sprite_ext *)state_add_drawable(state_id, layer, (draw_callback)draw_sprite_ext, &sprx, sizeof sprx);
}
