...
typedef enum EDITOR_MODE {
	MODE_RESIZE, MODE_TILEMAP, MODE_OBJECTS
} editor_mode;
...
typedef enum RESIZE_MODE {
	RESIZE_NONE, RESIZE_INNER, RESIZE_OUTER
} RESIZE_MODE;

/*********** TILE_MODE ***********/
typedef enum TILE_MODE {
	TILE_NONE, TILE_ADD, TILE_CLEAR
} TILE_MODE;

typedef enum TILE_BRUSH {
	TBRUSH_NORMAL, TBRUSH_DESTROYABLE
} TILE_BRUSH;
...
static char object_names[EDITOR_OBJECT_COUNT][32] = { "CRATE", "TANK",
        "FACTORY", "FACTORY", "FACTORY", "TURRET", "ROCKET", "ROBOTARM" };
static char param_names[EDITOR_OBJECT_COUNT][32] = { "def", "DEF", "DEF",
        "DEF_RED", "DEF_RAMP", "DEF", "DEF", "DEF" };

static char sprite_names[EDITOR_OBJECT_COUNT][32] = { "gear", "tankbody001",
        "factory_v2", "factoryred", "ramp", "turretgun001", "rocket", "saw" };
...
static editor_touch *add_touchdata(editor_mode m, touch_unique_id ID,
        cpVect view_start, cpVect offset, MODE_DATA data) {
...
static void update_instances(instance *obj, void *data) {
	level_add_object_recipe_name(lvl_tmpl, obj->TYPE->NAME,
	        (char*) &(((struct instance_dummy *) obj)->params), obj->p_start,
	        0);
}
...
static void tap_clear_editor(void *force) {
	if (!force) {
		SDL_ShowSimpleMessageBox(0, "Fjerne alt?",
		        "Sikker på at du vil fjerne alt?", NULL);
	}
...
static RESIZE_MODE radius_at_pos(cpVect pos, float margin) {
	...
	if (lvl_tmpl->inner_radius - margin < l
	        && l < lvl_tmpl->inner_radius + margin) {
		fprintf(stderr, "DEBUG: RESIZE_INNER\n");
		return RESIZE_INNER;
	} else if (lvl_tmpl->outer_radius - margin < l
	        && l < lvl_tmpl->outer_radius + margin) {
...
static void move_radius(editor_touch *touch) {
	...
	if (touch->data.resize_mode == RESIZE_INNER) {
		float new_r = lvl_tmpl->inner_radius
		        + (cpvlength(pos) - cpvlength(prev));
		if (new_r >= MIN_INNER_RADIUS
		        && new_r <= lvl_tmpl->outer_radius - MIN_RADIUS_OFFSET) {
			lvl_tmpl->inner_radius = new_r;
			grid_setregion2f(lvl_tmpl->tilemap.grid, lvl_tmpl->inner_radius,
			        lvl_tmpl->outer_radius);
			tilemap_updaterow(&lvl_tmpl->tilemap,
			        lvl_tmpl->tilemap.grid->pol.inner_i);
			tilemap_updaterow(&lvl_tmpl->tilemap,
			        lvl_tmpl->tilemap.grid->pol.inner_i + 1);
		}
		inner_resize_margin += (1 - inner_resize_margin) / 2;
	} else if (touch->data.resize_mode == RESIZE_OUTER) {
		float new_r = lvl_tmpl->outer_radius
		        + (cpvlength(pos) - cpvlength(prev));
		if (new_r <= MAX_OUTER_RADIUS
		        && new_r >= lvl_tmpl->inner_radius + MIN_RADIUS_OFFSET) {
			lvl_tmpl->outer_radius = new_r;
			grid_setregion2f(lvl_tmpl->tilemap.grid, lvl_tmpl->inner_radius,
			        lvl_tmpl->outer_radius);
			tilemap_updaterow(&lvl_tmpl->tilemap,
			        lvl_tmpl->tilemap.grid->pol.outer_i - 3);
			tilemap_updaterow(&lvl_tmpl->tilemap,
			        lvl_tmpl->tilemap.grid->pol.outer_i - 2);
			tilemap_updaterow(&lvl_tmpl->tilemap,
			        lvl_tmpl->tilemap.grid->pol.outer_i - 1);
			tilemap_updaterow(&lvl_tmpl->tilemap,
			        lvl_tmpl->tilemap.grid->pol.outer_i);
		}
...
static void paint_tile(editor_touch *touch) {
	...
	if (grid_i.yrow != -1) {
		if (touch->data.tile_mode == TILE_ADD) {
			tilemap_settile(&lvl_tmpl->tilemap, current_tlay, grid_i.xcol,
			        grid_i.yrow, 1, tilebrush == TBRUSH_DESTROYABLE);

			if (current_tlay == TLAY_SOLID
			        && !lvl_tmpl->tilemap.metadata[grid_i.yrow][grid_i.xcol].block) {
				cpVect verts[4];
				grid_getquad8cpv(lvl_tmpl->tilemap.grid, verts, grid_i.xcol,
				        grid_i.yrow);
				cpShape *shape = cpPolyShapeNew(current_space->staticBody, 4,
				        verts, cpvzero);
				lvl_tmpl->tilemap.metadata[grid_i.yrow][grid_i.xcol].block =
				        shape;
			...
			if (current_tlay == TLAY_SOLID
			        && lvl_tmpl->tilemap.metadata[grid_i.yrow][grid_i.xcol].block) {
				cpSpaceRemoveShape(current_space,
				        lvl_tmpl->tilemap.metadata[grid_i.yrow][grid_i.xcol].block);
				cpfree(
				        lvl_tmpl->tilemap.metadata[grid_i.yrow][grid_i.xcol].block);
				lvl_tmpl->tilemap.metadata[grid_i.yrow][grid_i.xcol].block =
	...
	if (touch->data.object_ins) {
		...
		if ((grid_i.yrow == -1)
			...
			if (is_deletable(touch->data.object_ins)) {
				...
				new_rad = (grid_i.dist_sq <= new_rad * new_rad) ? new_rad :
				grid_outer_radius(lvl_tmpl->tilemap.grid);
		...
		touch->data.object_ins = instance_at_pos(pos,
		OBJECT_MARGIN / view_editor->zoom, CP_ALL_LAYERS, CP_NO_GROUP);
	}
}
...
static int touch_down(SDL_TouchFingerEvent *finger) {
	...
	case MODE_OBJECTS:
	case MODE_RESIZE:
		data.object_ins = instance_at_pos(pos,
		OBJECT_MARGIN / view_editor->zoom, CP_ALL_LAYERS, CP_NO_GROUP);

		if (!data.object_ins) {
			data.resize_mode = radius_at_pos(pos,
			WALL_MARGIN / view_editor->zoom);
			...
			if (grid_i.yrow != -1) {
				byte tile =
				        lvl_tmpl->tilemap.data[current_tlay][grid_i.yrow][grid_i.xcol];
		...
		if (!contains_instance(data.object_ins)) {
			touch_unique_id touch_id = finger_bind(finger->fingerId);
			if (touch_id != -1) {
				editor_touch *touch = add_touchdata(MODE_OBJECTS, touch_id,
				        pos_view, cpvzero, data);
				if (data.object_ins) {
					...
					touch->data_obj_delete = is_deletable(
					        touch->data.object_ins)
					        && ((grid_i.yrow == -1)
					                || (pos.x
					                        < (panel_offset + PANEL_WIDTH
					                                - view_editor->view_width
					                                        / 2)));
				}
		...
		if (!contains_instance(data.object_ins)) {
			if (data.object_ins) {
				...
				editor_touch *touch = add_touchdata(MODE_OBJECTS, touch_id,
				        pos_view, cpvzero, data);
				touch->game_offset = cpvsub(data.object_ins->body->p, pos);
				touch->data_obj_delete =
				        is_deletable(touch->data.object_ins)
				                && ((grid_i.yrow == -1)
				                        || (pos.x
				                                < (panel_offset + PANEL_WIDTH
				                                        - view_editor->view_width
...
static int touch_motion(SDL_TouchFingerEvent *finger) {
	...
	if (touch) {
		...
		case MODE_OBJECTS:
			...
			if (!touch->data.object_ins
			        && (cpvdistsq(touch->viewpos_start, pos_view) > 20 * 20)) {
		...
		case MODE_OBJECTS:
			if (touch->data_obj_delete) {
				...
				instance_create(object_by_name(object_type),
				        level_get_param(&(param_defs), object_type, param_name),
				        pos, cpvzero);
				llist_remove(ll_touches, touch);
				pool_release(pool_touches, touch);
			} else {
...
static int editor_drag_button(button btn_id, SDL_TouchFingerEvent *finger,
        void *drag_data) {
	cpVect pos_view = cpv(finger->x, finger->y);
	cpVect pos = view_view2world(view_editor, pos_view);
	...
	if (offset >= DRAG_LIMIT) {
		...
		button_clear(btn_id);
		instance *ins = instance_create(object_by_name(object_type),
		        level_get_param(&(param_defs), object_type, param_name), pos,
		        cpvzero);
		touch_down(finger);
...
static void draw_gui(view *v) {
	touch_place(scr_objects, -GAME_WIDTH / 2 + PANEL_WIDTH / 2 + panel_offset,
	        0);
	...
	case MODE_RESIZE:
	case MODE_OBJECTS:
		draw_quad_patch_center(0, SPRITE_PLAYERGUN001, cpv(x, y), cpv(200, 200),
		        20, 0);
		bmfont_center(FONT_COURIER, cpv(0, -v->view_height / 2), 1, "MODE: %d",
		        (int) current_mode);
		draw_color4b(50, 50, 50, 25);
		draw_box(RLAY_GUI_BACK,
		        cpv(-GAME_WIDTH / 2 + panel_offset, -GAME_HEIGHT / 2),
		        cpv(PANEL_WIDTH, GAME_HEIGHT), 0, 0);
		break;
	case MODE_TILEMAP:
		bmfont_center(FONT_COURIER, cpv(0, -v->view_height / 2), 1,
		        "col: %d, row: %d", grid_i_cur.xcol, grid_i_cur.yrow);
		break;
	}
}
...
void editor_init() {
	...
	for (i = 0; i < 400; i++) {
		...
		if (we_randf < 0.1) {
			col2.a = 1;
			state_add_sprite(state_editor, layer, SPRITE_SPIKEBALL, size, size,
			        pos, 0, col2);
		} else {
			col2.r = (255 + col2.r) / 2;
			col2.g = (255 + col2.g) / 2;
			col2.b = (255 + col2.b) / 2;
			col2.a = 0;
			state_add_dualsprite(state_editor, layer, spr, pos, cpv(size, size),
			        col1, col2);
		}
	}
	...
	btn_space = button_create(SPRITE_BTN_HOME, 0, "", GAME_WIDTH / 2 - 120,
	GAME_HEIGHT * (0.90 - 0.5), 125, 125);
	btn_clear = button_create(SPRITE_BTN_RETRY, 0, "", GAME_WIDTH / 2 - 120,
	GAME_HEIGHT * (0.75 - 0.5), 125, 125);
	btn_test = button_create(SPRITE_BTN_NEXT, 0, "", GAME_WIDTH / 2 - 120,
	GAME_HEIGHT * (0.60 - 0.5), 125, 125);
	btn_save = button_create(SPRITE_COIN, 0, "", GAME_WIDTH / 2 - 120,
	GAME_HEIGHT * (0.45 - 0.5), 125, 125);
	btn_lvlname = button_create(0, 0, level_name, 0, GAME_HEIGHT / 2 - 50, 125,
	        125);

	btn_state_toggle = button_create(SPRITE_WHITE, 0, "", GAME_WIDTH / 2 - 200,
	GAME_HEIGHT * (0.2 - 0.5), 320, 100);
	btn_layer = button_create(NULL, 0, "", GAME_WIDTH / 2 - 25,
	        -GAME_HEIGHT / 2 + 25, 50, 50);
	btn_tile_toggler = button_create(NULL, 0, "X", GAME_WIDTH / 2 - 200,
	        -GAME_HEIGHT / 2 + 50, 100, 100);

	button_set_backcolor(btn_state_toggle, tilecol);

	button_set_click_callback(btn_state_toggle,
	        (btn_click_callback) editor_setmode, MODE_TOGGLE);
	button_set_click_callback(btn_layer, (btn_click_callback) setlayer,
	        btn_layer);
	button_set_click_callback(btn_tile_toggler,
	        (btn_click_callback) tilemode_toggle, btn_tile_toggler);
	...
	scr_world = scroll_create(0, 0, GAME_WIDTH, GAME_HEIGHT, 0.9, 3000, 1, 1,
	        0); // max 4 000 gu / sec
	scr_objects = scroll_create(-GAME_WIDTH / 2 + 150, 0, 300, GAME_HEIGHT, 0.9,
	        50, 0, 0, 1);
	scroll_set_callback(scr_world, touch_down, touch_motion, touch_up);
	scroll_set_bounds(scr_objects,
	        cpBBNew(0, -GAME_HEIGHT / 2, 0, EDITOR_OBJECT_COUNT * 200));
...
static void pre_update(void) {
	...
	if (current_mode == MODE_TILEMAP) {
		...
		if (current_mode == MODE_TILEMAP) {
			panel_offset =
			        panel_offset - panelspeed > -400 ?
			                panel_offset - panelspeed : -400;
		} else {
			panel_offset =
			        panel_offset + panelspeed < 0 ?
			                panel_offset + panelspeed : 0;
		}

		view_editor->zoom = scroll_get_zoom(scr_world);
		view_update(view_editor, cpvneg(scroll_get_offset(scr_world)),
		        scroll_get_rotation(scr_world));
		...
		while (llist_hasnext(ll_touches)) {
			...
			if (finger_status(touch->ID, -1)) {
				...
				case MODE_RESIZE:
					resize_inn =
					        touch->data.resize_mode == RESIZE_INNER ?
					                WE_TRUE : resize_inn;
					resize_out =
					        touch->data.resize_mode == RESIZE_OUTER ?
					                WE_TRUE : resize_out;
					move_radius(touch);
					break;
				}
		...
		if (!(resize_inn || resize_out)) {
			float target_ir =
			        lvl_tmpl->tilemap.grid->pol.rad[lvl_tmpl->tilemap.grid->pol.inner_i];
			float target_or =
			        lvl_tmpl->tilemap.grid->pol.rad[lvl_tmpl->tilemap.grid->pol.outer_i
			                - 1];
